...
socket.on("send subject",function(name,description)
{
	...
	$('#dbmsg').append("<?=$_SESSION[nick];?>"+"파일&nbsp"+ rows[j].title+"&nbsp을(를)"+"<br>" +rows[j].upload_date+ " 에 추가하였습니다."+ "<hr>");
	}
	});
...
socket.on("send member names",function(e)
{
	...
	socket.emit("best vote", id, name, token, room);
});
...
socket.on("send workshop contents", function(e)
{
	...
	for(var i =0;i<e.length; i++)
	{
		...
		$("<div id='cardThread'><input type='hidden' id='mp'/><input type='hidden' id='fp' value='"+e[i].file_no+"' /><img data-toggle='modal' data-target='#detailThread' id='cardImage' src='http://capstone.hae.so/v2/image_view.php?no="+e[i].file_no+"' style='cursor:pointer; border-top-left-radius:5px; border-top-right-radius:5px; border-bottom:1px solid #a9a9a9; max-width:100%; height:auto;' onclick='checkVote("+e[i].file_no+")'/><div id='content"+e[i].file_no+"' style='margin:6px 3px 0 3px; white-space:normal; font-size:14px; font-weight:500;'><div id = 'cardFileName'>"+e[i].title+"</div><br><span style='color:#aaaaaa; font-size:12px;' id='cardMembers'> "+e[i].nickname+"</span><br><br><span id='cardFiledescription' style='font-size:13px;'>"+e[i].description+"</span><br>★<span style='color:#666666; font-size:12px;'>"+e[i].score+"</span></div></div><br>").insertAfter($(".w3-card-12").eq(p-1).find('#inButton'));
	}
...
socket.on("view best vote",function(e)
{
	for(var i =0; i<e.length;i++)
	{
		//alert (e[i].file_no);
		$("#content10").append("<span class='glyphicon glyphicon-ok'>dfdfd</span>");
	}
});
...
$(function(){
	...
	{
		$('#loginStatus').append('<li><a data-toggle="modal" data-target="#myModal"><span style="color:white" class="glyphicon glyphicon-log-in" ></span> <span style="color:white;cursor:pointer;" onclick="location.href=\'http://capstone.hae.so/v2/login.php?go=<?="http://capstone.hae.so:10000/workshopKKD.php?no=".$_GET[no];?>\';"> 로그인</span></a></li><li><a href="register.php"><span class="glyphicon glyphicon-user" style="color:white"></span><span style="color:white"> 회원가입</span></a></li>');
	}
...
function vote ()
{
	...
	else
	{
		...
		updateBestVote (); // 버튼 누를 시, update best vote
	}
}

function updateBestVote ()
{
	socket.emit("best vote", id, name, token, room);
}
<?
