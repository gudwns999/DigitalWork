...
io.on('connection', function(client)
{
	...
	client.on("best vote", function(id, name, token, room, data)
	{
		if(isValidToken(id,name,token))
		{
			sql.query("select * from (SELECT dg_write.workshop_no, dg_write.process_no, dg_vote.file_no, COUNT(dg_vote.file_no) as cnt from dg_write, dg_vote where dg_write.workshop_no = dg_vote.workshop_no and dg_write.write_no = dg_vote.file_no GROUP BY dg_write.workshop_no, dg_write.process_no, dg_vote.file_no) tmp inner join (select workshop_no, process_no, file_no, cnt from (SELECT workshop_no, process_no, file_no, MAX(cnt) as cnt from (SELECT dg_write.workshop_no, dg_write.process_no, dg_vote.file_no, COUNT(dg_vote.file_no) as cnt from dg_write, dg_vote where dg_write.workshop_no = dg_vote.workshop_no and dg_write.write_no = dg_vote.file_no GROUP BY dg_write.workshop_no, dg_write.process_no, dg_vote.file_no) tmp2 GROUP BY process_no ORDER BY process_no ASC) tmp3) tmp4 on tmp.process_no=tmp4.process_no and tmp.cnt=tmp4.cnt", function(err, rows, fields)
			{
				io.sockets.in(room).emit("view best vote", rows);
				console.log (rows);
			});
		}
	});
