...
$room = $_GET[no];
$ret =  mysqli_fetch_array(mysqli_query($connect, "select * from dg_file_list where write_no={$_GET[no]}"),MYSQLI_BOTH);
...
$('#dbmsg').append("<?=$_SESSION[nick];?>님이"+"&nbsp파일&nbsp"+ rows[j].title+"&nbsp을(를)"+"<br>" +rows[j].upload_date.replace(/([A-Z]|\.[0]{3})/g," ")+"<br>"+" 에 추가하였습니다."+ "<hr>");
}
});
