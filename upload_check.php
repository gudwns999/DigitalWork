...
$arr = mysqli_fetch_array(mysqli_query($connect,"SELECT mem_no FROM dg_member WHERE email='{$_SESSION[id]}'"),MYSQLI_BOTH);
...
if(!$member_no) die("로그인 후 업로드 하실 수 있습니다.");
...
{
	alert("파일 첨부 도중 오류가 발생했습니다. 관리자에게 문의해 주세요. 에러코드 : ".$_FILES["fupload"]['error']);
}
...
$ableExt = array('jpg', 'jpeg', 'gif', 'png', 'bmp', 'stl', 'psd', 'ai', 'stl','pdf','txt','eps');
$path = pathinfo($_FILES["fupload"]['name']);
$ext = strtolower($path['extension']);
if (!in_array($ext, $ableExt))
	alert("현재 지원하지 않는 확장자입니다. 관리자에게 문의해 주세요.");

$upload_filename = $connect->real_escape_string($_FILES["fupload"]['name']);
...
move_uploaded_file($_FILES["fupload"]['tmp_name'], $folderName."/".$imgName);

complete($arr2["auto_increment"]);
function mres($a)
{
	global $connect;
