...
$name = $_POST[name];
$privated = $_POST[privated]=="true"?  1 : 0; 

$description = $_POST[description];
if(gettype($description)=="array") $description = "";
$type = $_POST[type];
$step = $tstep[step];
...
{
	if(mb_strlen($step[$i]) > 150) die("프로젝트 단계 중 ".($i+1)."단계의 이름을 150자 이내로 작성해 주세요.");
}
...
if($_POST[mod] == 1)
{
	...
	$query = "update dg_workshop set name='{$name}', description='{$description}', privated = '{$privated}', type='{$type}',master='{$_SESSION[id]}',fromdate='{$fromDate}',toDate='{$toDate}',process_cnt=".count($step)." where no={$no}";
...
else
{
	...
	//공방 테이블에 삽입할 쿼리
	$query = "insert into dg_workshop(name, description, privated, type, master, fromdate, todate, member_cnt, process_cnt) values('{$name}', '{$description}', '{$privated}', '{$type}', '{$_SESSION[id]}', '{$fromDate}', '{$toDate}', 1, ".count($step).");";
}
...
$masterno = mysqli_fetch_array(mysqli_query($connect, "select mem_no from dg_member where email='{$_SESSION[id]}'"),MYSQLI_BOTH);
if($_POST[mod] != 1)
{
	mysqli_query($connect, "insert dg_workshop_member(workshop_no, member_no) values({$ret[no]}, {$masterno['mem_no']})");
}
/*
//나머지 멤버에게는 초대를 한다.
