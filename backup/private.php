...
if($_GET[no])
{
	...
	if($_GET[mod]==1)
	{
		...
		if(!count($chk)) alert("공방 수정 권한이 없습니다.");
		else 
		{
			$mod = 1;
			$name = $chk[name];
			$private = $chk[privated];
			$type = $chk[type];
...
<div class="container" style="max-width:900px; border:1px solid #f9f9f9; background-color:white; padding:15px 15px 15px 15px;">
	...
	<form method="post" action="check_create_workshop.php" onsubmit="<?if($mod)echo "return modifyWorkshop();"; else echo "return createWorkshop();";?>">
		<div class="row" style="width:100%;">
			...
			<div class="col-sm-9">
				<input class="form-control" id="prjName" type="text" style="width:80%;"placeholder="생성할 공방의 이름을 적어주세요.(2~20 글자)" <?if($mod) echo "value='{$name}'";?>> <input type="checkbox" <?if($mod)if($private)echo"checked";?>>&nbsp;&nbsp;비공개&nbsp;&nbsp;
			</div>
		</div>
		<br>
		<div class="row" style="width:100%;">
		...
		<div class="row" style="width:100%;">
			...
			<div class="col-sm-9">
				...
				<button class="btn btn-default dropdown-toggle" type="button" data-toggle="dropdown"><?if($mod) echo $type; else{?>프로젝트 유형<?}?><span class="caret"></span></button>
				  <ul class="dropdown-menu">
					<li value="personal"><a href="#">개인 프로젝트</a></li>
					<li value="team"><a href="#">팀 프로젝트</a></li>
					<li value="school"><a href="#">학교 과제</a></li>
					<li value="contest"><a href="#">공모전</a></li>
					<li class="divider"></li>
