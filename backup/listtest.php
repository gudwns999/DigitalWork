...
// 일반적으로는 * 가 느리다.
$result_count=mysql_query("select * from dg_write",$conn);
...
<tr>
    ...
    <td height=20  bgcolor=white align=center>
        <a href=read.php?no=<?=$row['no']?>&no=<?=$no?>><?=$row['no']?></a>
    </td>
    <!-- 번호 끝 -->
    <!-- 제목 -->
    <td height=20  bgcolor=white>&nbsp;
        <a href=read.php?no=<?=$row['no']?>&no=<?=$no?>><?=strip_tags($row[write_name], '<b><i>');?></a>
    </td>
    <!-- 제목 끝 -->
    <!-- 이름 -->
    <td align=center height=20 bgcolor=white>
        <font  color=black>
            <a href="mailto:<?=$row[email]?>"><?=$row[member_no]?></a>
        </font>
    </td>
    <!-- 이름 끝 -->
