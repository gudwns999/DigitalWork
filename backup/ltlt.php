...
$result = $db->query("select * from dg_file_list");
$result2 = $db->query("select * from dg_write");
echo '<div id="file_list"><table>';
echo '<tr><th>no</th><th>파일명</th><th>저장파일명</th><th>타입</th><th>크기</th><th>넓이</th><th>높이</th><th>저장 IP</th><th>삭제</th><th>제목</th></tr>';
while( $row = $result->fetch_assoc() ) {
    $row2 = mysqli_fetch_array($result2,MYSQLI_BOTH);
    //이미지 정보 가져오기
    $file_server_path = realpath(__FILE__);
    $file_path =str_replace(basename(__FILE__), "", $file_server_path."KHJ/");
    $name = $file_path.$row['db_filename'];
    ...
    echo "<td>".htmlentities($row2['write_name'])."</td>";
    print '</tr>';
